...
export const getRESTIssues = async (
  ...
  const listQuery = queryParams.split(" ");
  for (let i = 0; i < listQuery.length; i++) {
    listQuery[i] = encodeURIComponent(listQuery[i]);
  }
  const parsedQuery = listQuery.length > 0 ? 'is%3Aopen' : listQuery.join("+");
  const response = await octokit.request(`GET /search/issues?q=${parsedQuery}`);
